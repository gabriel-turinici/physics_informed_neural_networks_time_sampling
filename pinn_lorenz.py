...
"""### Lambda constant class l=13.8 to have contrast 1000"""

pinn_lambda_constrast1k = PhysicsInformedNeuralNetwork(random_seed=1234,
                                                       sampling_lambda=np.log(1000)*2,weights_update_type=['lyapunov_lambda','causal','cst_lambda'][-1],
                                                       learning_rate=0.01)#too slow for 0.001, ok for lr=0.01 but still slower than Lyapunov

pinn_lambda_constrast1k.train()

pinn_lambda_constrast1k.plot_last_weights()
pinn_lambda_constrast1k.plot_loss_convergence()
pinn_lambda_constrast1k.plot_train_results()
...
loss_history_learning = pinn_learning.train()
...
pinn_causal = PhysicsInformedNeuralNetwork(random_seed=1234,
              weights_update_type=['lyapunov_lambda','causal','cst_lambda'][1],learning_rate=0.01)#random_seed=1234
niter=10000
...
#    plot_idx=iter_per_eps*ii+jj*int(iter_per_eps/plot_per_eps)
    plot_idx=1+iter_per_eps*ii+jj*int(iter_per_eps/(plot_per_eps-1))
    if(plot_idx <=len(pinn_causal.weights_history)-1):
#      plt.plot(pinn_causal.trange,pinn_causal.weights_history[plot_idx],label="iter "+str(jj*int(iter_per_eps/(plot_per_eps))),linewidth=2)
...
pinn0 = PhysicsInformedNeuralNetwork(random_seed=1234,sampling_lambda=0.0,
                                     weights_update_type=['lyapunov_lambda','causal','cst_lambda'][-1],learning_rate=0.01)#random_seed=1234
loss_history0 = pinn0.train(30000)
...
"""### Lambda constant class l=10.0"""

pinn10 = PhysicsInformedNeuralNetwork(random_seed=1234,sampling_lambda=10.0,
                                     weights_update_type=['lyapunov_lambda','causal','cst_lambda'][-1],learning_rate=0.01)#random_seed=1234
loss_history0 = pinn10.train(2000)

pinn10.plot_last_weights()
...
Test for Lyapunov 5000 iterations with learning rates of 0.001 and plot the two results on the same plot
"""

pinn_learning5k001 = PhysicsInformedNeuralNetwork(random_seed=1234,
                                             weights_update_type=['lyapunov_lambda','causal','cst_lambda'][0],learning_rate=0.001)#random_seed=1234
pinn_learning5k001.train(5000)

pinn_learning5k001.plot_train_results()

pinn_learning5k01 = PhysicsInformedNeuralNetwork(random_seed=1234,
                                             weights_update_type=['lyapunov_lambda','causal','cst_lambda'][0],learning_rate=0.01)#random_seed=1234
pinn_learning5k01.train(5000)

print('test case: ',pinn_learning5k01.test_case)
plt.figure('compare lr loss',figsize=(8,4))
plt.clf()
plt.semilogy(range(len(pinn_learning5k001.loss_list)), pinn_learning5k001.loss_list,linewidth=2,label='lr=0.001')
plt.semilogy(range(len(pinn_learning5k01.loss_list)), pinn_learning5k01.loss_list,linewidth=2,label='lr=0.01')
plt.ylabel("loss")
plt.xlabel("iteration")
plt.legend()
plt.tight_layout()
plt.savefig('lr_compare_losses_01_001_lorenz_lyapunov.pdf')
